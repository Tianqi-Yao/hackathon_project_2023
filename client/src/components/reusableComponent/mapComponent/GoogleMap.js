...
const GoogleMap = () => {
    ...
    const nearbySearchYelpFunc = (lat,lng) => {
        ...
        axios.get('http://localhost:3005/searchBusinessByYelp', {
            params: {
                lat: lat,
                lng: lng
            }
    ...
    const getMenuInfo = (url) => {
        ...
        axios.get('http://localhost:3005/api2', {
            params: {
                url: url
            }
        }).then((res) => {
    ...
    const getReviewsInfo = (url) => {
        console.log("getMenuInfo url: ", url);
        axios.get('http://localhost:3005/api3', {
            params: {
                url: url
            }
        }).then((res) => {
            ...
            <ul>food near me:
                {searchResultList?.map((item) => (
                    <li key={item.id}>
                        ...
                        <button onClick={()=>handleClickMenuBtn(item.url)}>get url menu</button>
                        <button onClick={()=>handleClickReviewsBtn(item.url)}>get url reviews</button>
                    </li>
                ))}
            </ul>
        </>
